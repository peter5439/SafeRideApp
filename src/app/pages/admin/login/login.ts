    ...
    <div class="min-h-screen bg-slate-900 flex items-center justify-center p-6">
      <div class="max-w-md w-full bg-white rounded-[40px] p-10 shadow-2xl">
        <div class="flex flex-col items-center text-center mb-10">
          ...
          <p class="text-slate-500">Secure access for SafeRide administrators</p>
        </div>
        ...
        <form (submit)="onSubmit($event)" class="space-y-4">
          ...
          <button type="submit" [disabled]="loading()" 
                  class="w-full bg-slate-900 text-white py-4 rounded-2xl font-bold shadow-lg active:scale-95 transition-all disabled:opacity-50 flex items-center justify-center gap-3 mt-4">
            @if (loading()) {
